...
pub fn valid_host(host: &str) -> bool {
    valid_ipv4_addr(host) || valid_ipv4_addr(host) || valid_hostname(host)
}
...
mod tests {
    ...
    fn command_cases() {
        ...
        assert!(valid_command("asdf"), "alphabetical is valid command");
        assert!(!valid_command("asd12"), "can't mix letters and numbers");
        assert!(valid_command("abCD"), "any case is fine");
    }

    #[test]
    fn tagval_cases() {
        assert!(valid_value("foobar"), "a word is valid",);
        assert!(!valid_value("foo bar"), "space is not allowed",);
        assert!(!valid_value("foo;bar"), "; is not allowed",);
        assert!(valid_value("foo😁bar"), "emoji is allowed",);
    }
    ...
    #[test]
    fn key_cases() {
        assert!(!valid_key("asd{f"), "key cannot contain a curly brace char",);
        assert!(valid_key("asd/f"), "key can contain /",);
        assert!(
            !valid_key("asd../f"),
